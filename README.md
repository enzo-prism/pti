...
- **Dr. Njo photo gallery**: `src/data/drNjoGallery.ts` is the curated storytelling set (Speaking & Authorship, Leadership & Community, Relationships) used by `/drnjo`, `/about`, `/events`, and the homepage; `src/data/galleryImages.ts` adds team/education photos and intrinsic dimensions for `/gallery`. Photos stand alone: do not render captions, figcaptions, or name bars. `caption` / `featuredImageCaption` fields in data files are unused inventory. Keep accurate `alt` text. `npm run images:check` fails if captions return to the UI. The same photos and the same testimonials are published on michaelnjodds.com — mirror additions to both sites.
- **Second edition of the handbook**: `/resources/second-book` (`src/views/SecondBook.tsx`) presents the forthcoming expanded second edition of *Dental Practice Transitions Handbook* (foreword by Dr. Glenn Vo). The Coming Soon pictorial also appears above the tools grid on `/resources`. No release date is published.

...
- **Community impact posts**: Recent photo/video updates live in `src/data/communityImpactPosts.ts` and are merged ahead of `blogPosts`. Photo posts should include a `featuredImage` so the hero is not a CSS gradient. Vertical graphics set `featuredImageAspect: "portrait"` and the real pixel size so the homepage Latest Update, blog cards, and post heroes do not letterbox them inside a landscape crop. Dates render through `formatLocalDate` — never the raw ISO string.

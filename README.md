...
- **Community impact posts**: Recent photo/video updates live in `src/data/communityImpactPosts.ts` and are merged ahead of `blogPosts`. Photo posts should include a `featuredImage` so the hero is not a CSS gradient. Portrait graphics (for example the Diana Fat Board of Regents card) set `featuredImageAspect: "portrait"` so the homepage Latest Update and blog cards do not letterbox them. Dates render through `formatLocalDate` — never the raw ISO string.
